...
ax = axes[0, 0]
ax.plot(t, d['rhalf_cold'], 'o-', color=CG, lw=2.2, ms=5, label=r'cold gas ($T<3\times10^4$ K)')
ax.plot(t, d['r90_cold'], 's--', color=CG, lw=1.4, ms=4, alpha=.6, label='cold gas, $R_{90}$')
...
# (b) robustness ---------------------------------------------------------------
ax = axes[0, 1]
for k, lab, c in [('rhalf_cold', r'$T<3\times10^4$ K', CG),
                  ('rhalf_cool', r'$T<10^4$ K', '#1a9850'),
...
# (c) gas budget ---------------------------------------------------------------
ax = axes[1, 0]
ax.plot(t, d['m_cold'] / 1e9, 'o-', color=CG, lw=2.2, ms=5,
        label=r'cold gas inside $R<30$, $|z|<3$ kpc')
...
# ------------------------------------------------------------------- numbers --
print('  t     R_half(cold)  R90    R_half(*)   M_cold[1e9]  M_out[1e9]')
for i in range(len(t)):
    print('  %4.1f  %11.2f %6.2f %10.2f %12.3f %11.3f'%(
        t[i], d['rhalf_cold'][i], d['r90_cold'][i], d['rhalf_star'][i],
        d['m_cold'][i]/1e9, d['m_cold_outside'][i]/1e9))
pre = (t >= 1.0) & (t < 1.6)
during = (t >= 1.6) & (t <= 3.2)
post = (t > 3.2) & (t <= 5.0)
late = t > 5.0
for lab, m in [('pre-merger  (1.0-1.6)', pre), ('during      (1.6-3.2)', during),
    ...
    print(f'{lab}: R_half(cold) = {np.nanmean(d["rhalf_cold"][m]):5.2f} kpc, '
          f'M_cold = {np.nanmean(d["m_cold"][m])/1e9:5.2f}e9, '
