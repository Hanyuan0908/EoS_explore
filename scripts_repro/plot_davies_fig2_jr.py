"""Davies+2025 Fig 2 planes ([Al/Fe]-[Fe/H] and [Mg/Fe]-[Fe/H]) for the Davies halo sample,
each pixel COLOUR-CODED by the median radial action J_R of the halo stars in it.
Actions via AGAMA ActionFinder in the McMillan (2017) potential; 6D from AstroNN by APOGEE_ID.
"""
import os
...
# 6D phase space from AstroNN VAC
...
Jr[sel] = J[:, 0]
print('halo with actions:', int(sel.sum()), ' J_R median=', np.nanmedian(Jr[sel]).round(0))

STAT = sys.argv[1] if len(sys.argv) > 1 else 'median'    # 'median' (robust) or 'mean'
XR = (-2.1, 0.6); NMIN = 3               # coarser bins below -> ~5 stars/cell in the sparse Eos region
VMIN, VMAX = 300, 1000                    # J_R colour range [kpc km/s]
def panel(ax, yv, yr, ylab, nb=(45, 35)):
    s = sel & np.isfinite(feh) & np.isfinite(yv)
    # per-bin statistic of J_R, masked below NMIN counts (mean is sensitive to the high-J_R tail)
    med = binned_statistic_2d(feh[s], yv[s], Jr[s], statistic=STAT, bins=nb, range=[XR, yr]).statistic
    ...
    ax.set_xlim(*XR); ax.set_ylim(*yr); label_axes(ax, '[Fe/H]', ylab,
                                                    f'Davies halo, coloured by {STAT} $J_R$ (n={int(s.sum())})')
    return im
...
cb = fig.colorbar(im, ax=ax, pad=0.01, shrink=0.9); cb.set_label(f'{STAT} ' + r'$J_R$ in bin [kpc km/s]')
fig.suptitle(f'Radial action ({STAT}) across the halo chemistry planes (AGAMA / McMillan 2017)', fontsize=12)
out = '01_davies_fig2_jr.png' if STAT == 'median' else f'01_davies_fig2_jr_{STAT}.png'
