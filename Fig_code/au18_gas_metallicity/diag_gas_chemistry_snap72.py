...
XLIM, ZLIM, NB, MMIN = 30., 24., 240, 5e5
...
def build():
    ...
    gp = np.column_stack([cg[:, 1], cg[:, 2], cg[:, 0]]) * 1e3
    ...
    np.savez(CACHE, W=Wm, xe=xe_, ze=ze_, GSE_x=G[:, 0], GSE_z=G[:, 2], **out)
...
if os.path.exists(CACHE):
    ...
    print(f'using cached maps: {CACHE}')
else:
