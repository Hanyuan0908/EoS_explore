"""Metallicity against birth radius for the two Eos populations in Au18.

R_birth is the cylindrical radius measured in the first stored snapshot at or
after each star formed (ana_birth_radii.py) -- the nearest-snapshot
approximation, with a snapshot spacing of ~0.15 Gyr, not an interpolation to the
exact formation time.  A star moving at 200 km/s covers ~30 kpc in 0.15 Gyr along
its orbit, so R_birth is reliable in the mean but noisy star by star; the running
medians below are the part to trust.
...
for lab, m, c in POPS:
    ax.hist(d['R_birth'][m], bins=bins, density=True, histtype='step', lw=2.2, color=c, label=lab)
    ax.axvline(np.nanmedian(d['R_birth'][m]), color=c, ls=':', lw=1.4)
ax.set(xlabel=r'$R_{\rm birth}$ [kpc]', ylabel='normalised density',
       title='(a) Birth radius')
ax.legend(fontsize=9)

# --- (b) the requested plane --------------------------------------------------
ax = axes[1]
ok = np.isfinite(d['R_birth'])
ax.hist2d(d['R_birth'][ok], d['feh'][ok], bins=(90, 80), range=((0, 25), (-1.6, 0.5)),
          norm=LogNorm(), cmap='Greys')
for lab, m, c in POPS[1:]:
    ax.plot(RCEN, running_median(d['R_birth'], d['feh'], m, REDGE, RCEN), 'o-',
...
for j, sn in enumerate(snaps_b):
    at = d['snap_birth'] == sn
    ax.plot(RCEN, running_median(d['R_birth'], d['feh'], at, REDGE, RCEN),
            '-', color=plt.cm.viridis(j / max(len(snaps_b) - 1, 1)), lw=1.1, alpha=.85,
            label='per-snapshot gradient' if j == 0 else None)
ax.plot(RCEN, running_median(d['R_birth'], d['feh'], np.ones(len(d['ids']), bool), REDGE, RCEN),
        's--', color='k', lw=1.8, ms=4, label='all merger-born, aggregate')
ax.set(xlim=(0, 25), xlabel=r'$R_{\rm birth}$ [kpc]', ylabel='[Fe/H]',
       title='(b) Metallicity vs birth radius')
ax.legend(fontsize=9)
...
for sn in snaps:
    at = np.isfinite(d['R_birth']) & (d['snap_birth'] == sn)
    for i in range(len(RCEN)):
        cell = at & (d['R_birth'] >= REDGE[i]) & (d['R_birth'] < REDGE[i + 1])
        if cell.sum() >= 15:
...
ax.set(xlabel=r'$\Delta$[Fe/H] vs stars born in the same snapshot and $R_{\rm birth}$ bin',
       ylabel='normalised density', title='(c) Gradient and epoch removed')
ax.legend(fontsize=8.5)

# --- (d) the birth plane ------------------------------------------------------
ax = axes[3]
ax.hist2d(d['R_birth'][ok], np.abs(d['z_birth'][ok]), bins=(90, 80),
          range=((0, 25), (0, 12)), norm=LogNorm(), cmap='Greys')
for lab, m, c in POPS[1:]:
    mm = m & ok
    step = max(1, mm.sum() // 2500)
    ax.scatter(d['R_birth'][mm][::step], np.abs(d['z_birth'][mm])[::step],
...
fig.suptitle('Au18: the two Eos populations against birth radius '
             r'($R_{\rm birth}$ = radius at the nearest snapshot)', fontsize=13)
fig.tight_layout(rect=[0, 0, 1, .94])
out = C.FIG_DIR + '/au18_eos_origins_rbirth.png'
fig.savefig(out, dpi=145)

print(f'{"":28s} {"N":>7s} {"R_birth":>9s} {"|z_birth|":>10s} {"[Fe/H]":>8s}')
for lab, m, c in POPS:
    print(f'{lab:28s} {m.sum():7,} {np.nanmedian(d["R_birth"][m]):9.2f} '
          f'{np.nanmedian(np.abs(d["z_birth"][m])):10.2f} {np.median(d["feh"][m]):+8.2f}')
a = running_median(d['R_birth'], d['feh'], d['halo_born'], REDGE, RCEN)
b = running_median(d['R_birth'], d['feh'], d['disc_born'], REDGE, RCEN)
both = np.isfinite(a) & np.isfinite(b)
print(f'\n[Fe/H] difference at matched R_birth (halo-born minus disc-born): '
      f'{np.mean((a - b)[both]):+.3f} dex over {both.sum()} shared bins')
allm = running_median(d['R_birth'], d['feh'], np.ones(len(d['ids']), bool), REDGE, RCEN)
...
for sn in snaps:
    at = d['snap_birth'] == sn
    med = running_median(d['R_birth'], d['feh'], at, REDGE, RCEN)
