"""Time evolution of the star-forming gas disc of any Auriga halo, through its merger.

Usage:  prep_gas_disc_auriga.py <halo> [snap ...]
        with no snapshot list, every snapdir present for that halo is used.

Generalised from prep_gas_disc_au18.py so the GS/E analogues identified by
Fattahi et al. (2019) can be compared.  Only the simulation directory changes
between halos; the cosmology, units and disc definition are shared.
...
HALO = int(sys.argv[1]) if len(sys.argv) > 1 else 18
C.SIM_DIR = f'/home/hz420/austreams/Auriga_simulation/halo_{HALO}/'
C.HALO = C.HALO_TAG = f'halo_{HALO}'

RMAX, ZMAX = 30., 5.
RMAX_W, ZMAX_W = 50., 10.
T_COLD = 3e4
XH, GAMMA, MP, KB = 0.76, 5. / 3., 1.6726219e-24, 1.380649e-16
SNAPS = [int(x) for x in sys.argv[2:]]
if not SNAPS:
    SNAPS = sorted(int(p.rsplit('_', 1)[1]) for p in glob.glob(C.SIM_DIR + 'snapdir_*'))
print(f'halo_{HALO}: {len(SNAPS)} snapshots, {SNAPS[0]}-{SNAPS[-1]}', flush=True)
...
np.savez(C.OUT_DIR + f'/gas_disc_evolution_au{HALO}.npz', **{k: np.array(v) for k, v in rec.items()})
print('\nsaved', C.OUT_DIR + f'/gas_disc_evolution_au{HALO}.npz')
